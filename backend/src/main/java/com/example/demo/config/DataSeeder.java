    ...
    public void run(String... args) throws Exception {
        ...
        try {
            ...
            if (newAdmin.isPresent()) {
                Admin admin = newAdmin.get();
                admin.setPassword("Rajaji@132134");
                admin.setRole("SUPER_ADMIN");
                adminRepository.save(admin);
                System.out.println("Super Admin 'rajaji.gh@gmail.com' password updated to 'Rajaji@132134'");
                
                if (oldAdmin.isPresent()) {
                    adminRepository.delete(oldAdmin.get());
                    System.out.println("Cleaned up old admin account to prevent duplicates");
                }
            } else if (oldAdmin.isPresent()) {
                Admin admin = oldAdmin.get();
                admin.setEmail("rajaji.gh@gmail.com");
                admin.setPassword("Rajaji@132134");
                admin.setRole("SUPER_ADMIN");
                adminRepository.save(admin);
                System.out.println("Super Admin email migrated to: rajaji.gh@gmail.com and password updated to 'Rajaji@132134'");
            } else {
                Admin defaultAdmin = new Admin();
                defaultAdmin.setName("System Administrator");
                defaultAdmin.setEmail("rajaji.gh@gmail.com");
                defaultAdmin.setPhone("9999999999");
                defaultAdmin.setPassword("Rajaji@132134");
                defaultAdmin.setRole("SUPER_ADMIN");
                defaultAdmin.setStatus("ACTIVE");
                adminRepository.save(defaultAdmin);
                System.out.println("Default Super Admin created: email='rajaji.gh@gmail.com' with password 'Rajaji@132134'");
            }
        } catch (Exception e) {
